...
| Abilita canale | Decide se il canale partecipa alla scelta del caricamento. | Consigliato |
...
| Path-style access | Opzione di compatibilità, disattivata per impostazione predefinita. Alcuni servizi MinIO, OpenList o S3 autogestiti possono richiederla. | No |
...
![Voce token API R2](../../image/upload/s3/r2%20api%20token入口.png)
...
| Caricamento funzionante | Carica un'immagine di prova e verifica che l'oggetto compaia nel bucket. |
| Link apribile | Il link restituito dopo il caricamento si apre normalmente. |
...
Attivalo solo se il provider S3 compatibile non supporta URL virtual-hosted-style o se la documentazione lo richiede. Alcuni endpoint MinIO, S3 autogestiti e compatibili OpenList possono averne bisogno.
...
### Perché il caricamento fallisce?
