...
> ImgBed を初めて使用する前に、初期化ページで「インデックスを再構築」をクリックし、必要な D1 データテーブルを補完して、後続機能のエラーを避けてください。
>
> ![初期化時にインデックスを再構築をクリック](../../image/初始化点击重建索引.png)

- 使用するストレージアカウントまたは API 認証情報を用意します。
- OAuth ベースのチャネルではコールバック URL が必要になるため、ImgBed のドメインに到達できることを確認します。
